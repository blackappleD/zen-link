...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                var defaults = {
                    ...
                    pageSize: 15,
                    pageNumber: 1,
                    pageList: [5, 10, 15, 20, 50, 100, 200, 500],
                    toolbar: "#toolbar",
                    loadingFontSize: 13,
                    striped: false,
                    escape: false,
                ...
                $.table.initEvent();
                $('#' + options.id).bootstrapTable($.extend({
                    id: options.id,
            ...
            queryParams: function(params) {
                ...
                var curParams = {
                    ...
                    isAsc:          params.order,
                    isFormReset:    params.isFormReset
                };
            ...
            initEvent: function() {
                ...
                $(optionsIds).on("click", '.tooltip-show', function() {
                    ...
                    if ($.common.equals("copy", target)) {
                        ...
                        top.layer.alert(input.val().replace(/</g, '&lt;'), {
                            title: "信息内容",
                            shadeClose: true,
                            btn: ['复制'],
            ...
            tooltip: function (value, length, target) {
                ...
                if (_value.length > _length) {
                    ...
                    _value = _value.replace(/\"/g,"&quot;");
                    var actions = [];
                    // width:50px 宽度从5改为50是为了能复制，不然复制不起作用
                    actions.push($.common.sprintf('<input style="opacity: 0;position: absolute;width:50px;z-index:-1" type="text" value="%s"/>', _value));
        ...
        modal: {
            ...
            openOptions: function (options) {
                ...
                if ($.common.isEmpty(options.callBack)) {
                    options.callBack = function(index, layero, callback) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.submitHandler(index, layero, callback);
                    }
                }
                if (options.yes !== false && !$.isFunction(options.yes)) {
                    options.yes = options.btn1 || options.callBack;
                }
                if (!$.common.isEmpty(options.btn1)) {
                    delete options.btn1;
                }
        ...
        operate: {
            ...
            // 保存信息 刷新表格
            saveUploadFile: function(url, data, callback) {
                $.modal.loading("正在处理中，请稍候...");
                $.modal.disable();
        ...
        common: {
            ...
            formToJSON: function(formId) {
                ...
                $('select[sort]').each((i, s) => {
					$(':selected', s).each((j, o) => {
						var eleVal = formArr.find(e => e.name == s.name && e.value == o.value);
						if(eleVal) eleVal.sindex = o.sindex;
					});
				});
				formArr.sort((c, n) => c.sindex - n.sindex);
