...
function initTwoDate(startTime, endTime) {
	layui.use('laydate', function() {
        var laydate = layui.laydate;
        var $startTime = $(startTime || '#startTime').attr('autocomplete', 'off');
        var $endTime = $(endTime || '#endTime').attr('autocomplete', 'off');
        var startLayDt = laydate.render({
            elem: $startTime[0],
            max: $endTime.val() || '2099-12-31',
            theme: 'molv',
            type: $startTime.attr("data-type") || 'date',
            format: $startTime.attr("data-format") || 'yyyy-MM-dd',
            trigger: 'click',
            showBottom: $startTime.attr("data-show-bottom") != 'false',
            done: function(value, date) {
                // 结束时间大于开始时间
                if (value !== '') {
                    endLayDt.config.min.year = date.year;
                    endLayDt.config.min.month = date.month - 1;
                    if(endLayDt.config.type.indexOf('date') != -1) endLayDt.config.min.date = date.date;
                    
                    var interval = Number($startTime.attr("data-interval"));
                    if(interval) {
						var d = new Date(value);
						d.setDate(d.getDate() + interval);
						endLayDt.config.max.year = d.getFullYear();
                        endLayDt.config.max.month = d.getMonth();
                        endLayDt.config.max.date = d.getDate();
					}
                } else {
                    endLayDt.config.min.year = '';
                    endLayDt.config.min.month = '';
                    endLayDt.config.min.date = '';
                    
                    endLayDt.config.max.year = '2099';
                    endLayDt.config.max.month = '12';
                    endLayDt.config.max.date = '31';
                }
                if($startTime.attr("data-trigger-end-time") != 'false') $endTime.trigger('click');
            }
        });
        if(!startTime) startLayDate = startLayDt;
        
        var endLayDt = laydate.render({
            elem: $endTime[0],
            min: $startTime.val() || '1900-1-1',
            theme: 'molv',
            type: $endTime.attr("data-type") || 'date',
            format: $endTime.attr("data-format") || 'yyyy-MM-dd',
            trigger: 'click',
            showBottom: $endTime.attr("data-show-bottom") != 'false',
            done: function(value, date) {
                // 开始时间小于结束时间
                if (value !== '') {
                    startLayDt.config.max.year = date.year;
                    startLayDt.config.max.month = date.month - 1;
                    startLayDt.config.max.date = date.date;
                    
                    var interval = Number($endTime.attr("data-interval"));
                    if(interval) {
						var d = new Date(value);
						d.setDate(d.getDate() - interval);
						startLayDt.config.min.year = d.getFullYear();
                        startLayDt.config.min.month = d.getMonth();
                        startLayDt.config.min.date = d.getDate();
					}
                } else {
                    startLayDt.config.max.year = '2099';
                    startLayDt.config.max.month = '12';
                    startLayDt.config.max.date = '31';
                    
                    startLayDt.config.min.year = '';
                    startLayDt.config.min.month = '';
                    startLayDt.config.min.date = '';
                }
            }
        });
        if(!endTime) endLayDate = endLayDt;
    });
}
