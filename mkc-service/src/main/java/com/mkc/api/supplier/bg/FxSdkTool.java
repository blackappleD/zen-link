package com.mkc.api.supplier.bg;

import cn.hutool.json.JSONUtil;
import com.alibaba.fastjson2.JSONObject;
import com.gzy.sign.api.*;
import com.gzy.sign.api.config.FaXinApiClientConfig;
import com.gzy.sign.api.request.FaXinFileStreamUploadApiRequest;
import com.gzy.sign.api.request.FaXinHouseVerifyApiRequest;
import com.gzy.sign.api.request.FaXinHouseVerifyResultApiRequest;
import com.gzy.sign.api.response.FaXinHouseVerifyApiResponse;
import com.gzy.sign.api.response.FaXinHouseVerifyResultApiResponse;
import com.mkc.api.vo.bg.MaterialReqVo;
import com.mkc.api.vo.bg.PersonInfoReqVo;

import java.io.File;
import java.util.ArrayList;
import java.util.List;

public class FxSdkTool {

    //private final static FaXinDefaultApiClient client = getDefaultApiClient();

    private final static String appId = "936179597611134976";

//    public static String privateKey = "";
//
//    public static String publicKey = "";

    private static FaXinDefaultApiClient getDefaultApiClient(String privateKey, String publicKey) {
        //平台方openApi的地址
        String baseServerUrl = "https://uat-fq-api.esnotary.com/";
//        String baseServerUrl = "https://file-api.esnotary.com/";
        //平台方openApi文件服务的地址
        String fileUploadBaseServerUrl = "https://uat-file-api.esnotary.com/";
//        String fileUploadBaseServerUrl = "https://file-api.esnotary.com/";
        //平台方分配的appId
        // String appId = "670099866900279296_test";
       // String appId = "936179597611134976";
        //合作方的私钥
        //String privateKey = "";
        //平台方提供公钥
       // String publicKey = "";

        FaXinApiClientConfig config = new FaXinApiClientConfig(baseServerUrl, fileUploadBaseServerUrl, appId, privateKey, publicKey, "RSA2", "RSA2");
        return new FaXinDefaultApiClient(config);
    }

//    public static <T extends AbstractFaXinResponse> T executeRequestSuccess(FaXinApiRequest<T> request) {
//        //合作方自己封装的请求，只返回成功，失败的直接抛异常，由合作方决定是否使用
//        System.out.println("请求对象:" + request);
//        FaXinResponseResultInfo<T> responseResultInfo = executeRequest(request);
//        System.out.println("响应结果:" + responseResultInfo);
//        if (StringUtil.notEqualsIgnoreCase(responseResultInfo.getCode(), "E00000")) {
//            //合作方自定义异常
//            throw new BaseFaXinApiException(responseResultInfo.getMessage());
//        }
//        System.out.println("响应业务参数:" + responseResultInfo.getData());
//        return responseResultInfo.getData();
//    }

    public static <T extends AbstractFaXinResponse> FaXinResponseResultInfo<T> executeRequest(FaXinApiRequest<T> request, String privateKey, String publicKey) {
        //合作方自己封装的请求，不区分返回结果，由合作方决定是否使用
        long s = System.currentTimeMillis();
        FaXinDefaultApiClient client = getDefaultApiClient(privateKey, publicKey);
        FaXinResponseResultInfo<T> resultInfo = client.executeRequest(request);
        System.out.println("请求耗时：" + (System.currentTimeMillis() - s) + "ms");
        return resultInfo;
    }

//    public static FaXinCallbackBusinessDataResultInfo executeCallback(FaXinCallbackApiRequest callbackApiRequest) {
//        return client.executeCallback(callbackApiRequest);
//    }

//    public static FaXinCallbackApiResponse executeCallbackSuccessResp(String requestId) {
//        return client.executeCallbackResponse("E00000", "成功", requestId);
//    }

//    public static FaXinCallbackApiResponse executeCallbackFailResp(String msg, String requestId) {
//        return client.executeCallbackResponse("E99999", msg, requestId);
//    }

    /**
     * 单文件上传
     */
    public static String uploadFile(File file, String privateKey, String publicKey){
        //File file = new File("E:\\文件3.jpg");
        FaXinFileStreamUploadApiRequest apiRequest = new FaXinFileStreamUploadApiRequest(file, null, file.getName());
        //文件上传请求的响应实体类
        //FaXinFileStreamUploadApiResponse fileStreamUploadApiResponse = (FaXinFileStreamUploadApiResponse) executeRequest(apiRequest);
        FaXinResponseResultInfo<FaXinHouseVerifyApiResponse> faXinResponseResultInfo = executeRequest(apiRequest, privateKey, publicKey);
        //return fileStreamUploadApiResponse.getFileId();
        return JSONUtil.toJsonStr(faXinResponseResultInfo);
    }

    public static void main(String[] args) {
//        File file = new File("F:\\1.png");
//        FaXinFileStreamUploadApiRequest apiRequest = new FaXinFileStreamUploadApiRequest(file, null, file.getName());
//        //文件上传请求的响应实体类
//        FaXinFileStreamUploadApiResponse fileStreamUploadApiResponse = (FaXinFileStreamUploadApiResponse) executeRequestSuccess(apiRequest);
//        houseCheck();
//        houseCheckResult();
    }

    /***
     *不动产信息核查接口
     */
    public static String houseCheck(JSONObject jsonObject, String privateKey, String publicKey) {
        FaXinHouseVerifyApiRequest faXinHouseVerifyApiRequest = new FaXinHouseVerifyApiRequest();
        JSONObject data = jsonObject.getJSONObject("data");
        List<PersonInfoReqVo> persons = data.getList("persons", PersonInfoReqVo.class);
        List<FaXinHouseVerifyApiRequest.PersonDTO> personDTOList = new ArrayList<>();
        for (PersonInfoReqVo reqVo : persons) {
            FaXinHouseVerifyApiRequest.PersonDTO personDTO = new FaXinHouseVerifyApiRequest.PersonDTO();
            personDTO.setName(reqVo.getName());
            personDTO.setCardNum(reqVo.getCardNum());
            personDTOList.add(personDTO);
        }
        List<FaXinHouseVerifyApiRequest.MaterialDTO> materialDTOList = new ArrayList<>();
        List<MaterialReqVo> materials = data.getList("materials", MaterialReqVo.class);
        for (MaterialReqVo materialReqVo : materials) {
            FaXinHouseVerifyApiRequest.MaterialDTO materialDTO = new FaXinHouseVerifyApiRequest.MaterialDTO();
            materialDTO.setMaterialType(materialReqVo.getType());
            materialDTO.setMaterialFileId(materialReqVo.getFileId());
            materialDTOList.add(materialDTO);
        }
        faXinHouseVerifyApiRequest.setPersons(personDTOList);
        faXinHouseVerifyApiRequest.setMaterials(materialDTOList);
        FaXinResponseResultInfo<FaXinHouseVerifyApiResponse> faXinResponseResultInfo = executeRequest(faXinHouseVerifyApiRequest, privateKey, publicKey);
//        FaXinHouseVerifyApiResponse houseVerifyApiResponse = faXinResponseResultInfo.getData();
//        HouseInfoReqReturnVo reqReturnVo = new HouseInfoReqReturnVo();
//        reqReturnVo.setCode(faXinResponseResultInfo.getCode());
//        reqReturnVo.setReqOrderNo(houseVerifyApiResponse.getReqOrderNo());
        return JSONUtil.toJsonStr(faXinResponseResultInfo);
    }



    /**
     * 不动产信息核查结果接口
     */
    public static String houseCheckResult(JSONObject jsonObject, String privateKey, String publicKey) {
        JSONObject data = jsonObject.getJSONObject("data");
        FaXinHouseVerifyResultApiRequest faXinHouseVerifyResultApiRequest = new FaXinHouseVerifyResultApiRequest();
        faXinHouseVerifyResultApiRequest.setReqOrderNo(data.getString("reqOrderNo"));
        List<String> personCardNumList = data.getList("personCardNumList", String.class);
        List<FaXinHouseVerifyResultApiRequest.PersonCardNum> cardNumList = new ArrayList<>();
        for (String cardNum : personCardNumList) {
            FaXinHouseVerifyResultApiRequest.PersonCardNum personCardNum = new FaXinHouseVerifyResultApiRequest.PersonCardNum();
            personCardNum.setCardNum(cardNum);
            cardNumList.add(personCardNum);
        }
        faXinHouseVerifyResultApiRequest.setPersonCardNumList(cardNumList);
        FaXinResponseResultInfo<FaXinHouseVerifyResultApiResponse> faXinResponseResultInfo = executeRequest(faXinHouseVerifyResultApiRequest, privateKey, publicKey);
//        String code = faXinResponseResultInfo.getCode();
//        FaXinHouseVerifyResultApiResponse houseVerifyApiResponse = faXinResponseResultInfo.getData();
//        System.out.println(code);
//        System.out.println(houseVerifyApiResponse);
        return JSONUtil.toJsonStr(faXinResponseResultInfo);
    }


}
