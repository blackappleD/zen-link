...
import com.mkc.api.dto.ck.req.PopulationTwoReqDTO;
import com.mkc.api.supplier.ICkSupService;
import com.mkc.api.dto.ck.req.PopulationThreeReqDTO;
...
public class CdDataGroupCkSupImpl implements ICkSupService {
	...
	@Override
	public SupResult ckPopulationTwo(PopulationTwoReqDTO vo, SuplierQueryBean bean) {
		...
		String url = null;
		try {
			url = bean.getUrl() + "/xs-api/xs-data-api/api/proxy/a197180d3fc8789fe";
			String appkey = bean.getSignKey();
			String appsecret = bean.getSignPwd();
			Integer timeOut = bean.getTimeOut();
			String name = vo.getName();
			...
			if (SUCCESS.equals(code)) {
				...
				supResult.setState(ReqState.ERROR);
				supResult.setDefinedFailMsg(true);
				errMonitorMsg(log, " 【成都数据集团股份有限公司】 全国⼈⼝身份信息二要素核验 接口 发生异常 orderNo {} URL {} , 报文: {} "
						, bean.getOrderNo(), url, result);
				return supResult;
			} else if (ERROR2.equals(code)) {
				supResult.setRemark(resultObject.getString("msg"));
				...
				supResult.setRemark("查询失败");
				supResult.setState(ReqState.ERROR);
				errMonitorMsg(log, "  全国⼈⼝身份信息二要素核验 接口 发生异常 orderNo {} URL {} , 报文: {} "
						, bean.getOrderNo(), url, result);
				return supResult;
			}
			return supResult;
		} catch (Throwable e) {
			errMonitorMsg(log, " 【成都数据集团股份有限公司】 全国⼈⼝身份信息二要素核验 接口 发生异常 orderNo {} URL {} , 报文: {} , err {}"
					, bean.getOrderNo(), url, result, e);
			if (supResult == null) {
				supResult = new SupResult(params.toJSONString(), LocalDateTime.now());
			}
